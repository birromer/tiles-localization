...
float size_carrelage_x = 0.30;
float size_carrelage_y = 0.30;

float percent = 0.8; //de combien max on estime qu'on aura bougé (là, de moins d'un carreau donc le calcul est possible)
...
void calc_new_pos(std::vector<Vec4i> lines){
	...
	for(int i=0; i<lines.size(); i++) {
		...
		//translation pour centrer les pointsd autour de 0
	    ...
	    //cout << "alpha lignes : " << alpha*180/M_PI << " | " << angle*180/M_PI << endl;
		...
		line(rot, cv::Point(x1, y1), cv::Point(x2, y2), Scalar(0, 0, 255), 1, CV_AA);
	...
	alpha_median = alpha_median ;//+ M_PI/2.;

	cout << "alpha_median : " << alpha_median << endl;
	...
	*/
    ...
    float medx = median(Mew);
    float medy = median(Msn);
	...
	*/
	...
	//X[2] = Interval(fmod(alpha_median-0.2, 2*M_PI), fmod(alpha_median+0.2, 2*M_PI));
	//X[2] = Interval(State[2].mid()-0.1, State[2].mid()+0.1);
	X[2] = Interval(State[2].mid()).inflate(0.1);
	...
	box2[0] = X[0], box2[1] = X[1], box2[2] = X[2], box2[3] = Interval(medx).inflate(0.03), box2[4] = Interval(medy).inflate(0.03), box2[5] = Interval(alpha_median).inflate(0.1); //X[2];
	box3[0] = X[0], box3[1] = X[1], box3[2] = X[2], box3[3] = Interval(medx).inflate(0.03), box3[4] = Interval(medy).inflate(0.03), box3[5] = Interval(alpha_median).inflate(0.1); //X[2];
	...
	cout << "box1 0 : " << box0 << endl;


	c1.contract(box0);
	cout << "box0 1 : " << box0 << endl;
	cout << "box1 1 : " << box0 << endl;

	c2.contract(box1);
	cout << "box0 2 : " << box0 << endl;
	cout << "box1 2 : " << box0 << endl;
...
void imageCallback(const sensor_msgs::ImageConstPtr& msg)
{
  try
  {
    ...
    cv::HoughLinesP(edges, lines, 1, CV_PI/180., 20, 150, 250);
    ...
    for (size_t i=0; i<lines.size(); i++) 
    {
        ...
        if (fabs(angle_line) < M_PI/3) // On filtre les lignes perpendiculaires
        {
            ...
            n += 1;
            sumPos += (y1+y2)/2 ; 
            cv::line(out, cv::Point(l[0], l[1]), cv::Point(l[2], l[3]), cv::Scalar(255, 0, 0), 3, CV_AA);
        }
        else
        {
            cv::line(out, cv::Point(l[0], l[1]), cv::Point(l[2], l[3]), cv::Scalar(0, 0, 255), 3, CV_AA); 
        }
        //std::cout<< angle_line*180/M_PI<< std::endl;
    }
	**/
