...
void calc_new_pos(std::vector<Vec4i> lines){
	...
	*/
	...
	ibex::Function f1("x[3]", "y[3]", "(sin(pi*(x[0]-y[0])) ; sin(pi*(x[1]-y[1])) ; cos(x[2]-y[2]))");
	ibex::Function f2("x[3]", "y[3]", "(sin(pi*(x[0]-y[1])) ; sin(pi*(x[1]-y[0])) ; sin(x[2]-y[2]))");
